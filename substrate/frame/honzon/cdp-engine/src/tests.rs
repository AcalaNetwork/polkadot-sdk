...
use frame_support::{
	...
	traits::{fungible::MutateHold, honzon::Change},
};
...
fn set_collateral_params_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(CDPEngine::set_collateral_params(
			RuntimeOrigin::root(),
			Change::NoChange,
			Change::NewValue(Some(Ratio::from_inner(1_800_000_000_000_000_000u128))),
			Change::NoChange,
			Change::NewValue(Some(Ratio::from_inner(2_500_000_000_000_000_000u128))),
			Change::NewValue(2_000_000u128),
		));

		let params = CDPEngine::collateral_params();
		assert_eq!(
		...
		assert_eq!(params.maximum_total_debit_value, 2_000_000u128);
	});
}
