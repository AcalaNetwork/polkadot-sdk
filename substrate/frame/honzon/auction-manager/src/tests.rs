...
fn collateral_auction_methods() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			...
			Some(pallet_auction::AuctionInfo {
				bid: None,
				start: 0,
				end: Some(2000)
			})
...
fn new_collateral_auction_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			AuctionManagerModule::new_collateral_auction(&ALICE, BTC, mock::Balance::max_value(), mock::Balance::max_value()),
			Error::<Runtime>::InvalidAmount,
		);
	});
}
