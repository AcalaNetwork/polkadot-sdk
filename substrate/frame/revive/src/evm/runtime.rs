...
use crate::{
	...
	AccountIdOf, AddressMapper, BalanceOf, Config, DispatchClass, MomentOf,
	OnChargeTransactionBalanceOf, Pallet, Zero, LOG_TARGET, RUNTIME_PALLETS_ADDR,
};
use alloc::vec::Vec;
...
use pallet_transaction_payment::{Config as TxConfig, NextFeeMultiplier};
...
pub trait EthExtra {
		...
		<Self::Config as TxConfig>::WeightToFee: BlockRatioWeightToFee<T = Self::Config>,
	{
		...
		let extrinsic_fee: BalanceOf<Self::Config> = pallet_transaction_payment::Pallet::<
			Self::Config,
		>::compute_fee(
			encoded_len as u32, &info, 0u32.into()
		)
		.into();
		...
		let remaining_fee = eth_fee.checked_sub(extrinsic_fee.into()).ok_or_else(|| {
			log::debug!(target: LOG_TARGET, "Not enough gas supplied to cover the extrinsic base fee");
			InvalidTransaction::Payment
		})?;

		let weight_limit = {
			let remaining_unadjusted_fee = <NextFeeMultiplier<Self::Config>>::get()
				.reciprocal()
				.ok_or_else(|| {
					log::debug!(target: LOG_TARGET, "Gas price dropped to zero. This is a bug.");
					InvalidTransaction::Payment
				})?
				.saturating_mul_int(<BalanceOf<Self::Config>>::saturated_from(remaining_fee));
			let weight_limit = <Self::Config as TxConfig>::WeightToFee::fee_to_weight(
				remaining_unadjusted_fee.saturated_into(),
			);
		...
		let mut info = call.get_dispatch_info();
		info.extension_weight = Self::get_eth_extension(nonce, 0u32.into()).weight(&call);
		let tx_fee: BalanceOf<Self::Config> =
		...
		<Self::Config as Config>::Currency::hold(
			&<Self::Config as Config>::DepositSource::get().unwrap(),
			&signer,
			storage_deposit,
		)
		.map_err(|_| {
