...
pub mod pallet {
		...
		T: pallet_transaction_payment::Config,
	{
		...
		fn integrity_test() {
			...
			assert!(
				T::FeeMultiplierUpdate::min().saturating_mul_int(T::NativeToEthRatio::get()) > 0,
	...
	T::Hash: frame_support::traits::IsType<H256>,
{
	...
	pub fn dry_run_eth_transact(
		...
		gas_limit: Weight,
		tx_fee: impl Fn(<T as Config>::RuntimeCall, <T as Config>::RuntimeCall) -> Option<BalanceOf<T>>,
	) -> Result<EthTransactInfo<BalanceOf<T>>, EthTransactError>
	where
		<T as frame_system::Config>::RuntimeCall:
			Dispatchable<Info = frame_support::dispatch::DispatchInfo>,
		OnChargeTransactionBalanceOf<T>: Into<U256>,
		...
		T::Hash: frame_support::traits::IsType<H256>,
	{
		...
		let effective_gas_price = tx.effective_gas_price(base_fee).unwrap_or_default();
		...
		if tx.max_priority_fee_per_gas.is_none() {
			tx.max_priority_fee_per_gas = Some(base_fee);
		}
		...
		let extrinsic_fee: U256 = tx_fee(eth_transact_call.into(), dispatch_call)
			.ok_or_else(|| {
				EthTransactError::Message("Gas price dropped to zero. This is a bug".to_string())
			})?
			.into();
		let gas_limit: U256 =
			<pallet_transaction_payment::Pallet<T>>::weight_to_fee(weight_limit).into();
		...
		let eth_gas: U256 = extrinsic_fee
			.saturating_add(gas_limit)
			.saturating_add(result.storage_deposit.into())
			.into();

		log::debug!(target: LOG_TARGET, "\
			dry_run_eth_transact: \
			weight_limit={weight_limit:?}: \
			(extrinsic_fee={extrinsic_fee:?}) + (gas_limit={gas_limit:?}) + (storage_deposit={:?}) = (eth_gas={eth_gas:?})\
			",
	...
	where
		OnChargeTransactionBalanceOf<T>: Into<U256>,
	{
		pallet_transaction_payment::Pallet::<T>::weight_to_fee(weight).into()
	}
}
			...
			impl pallet_revive::ReviveApi<Block, AccountId, Balance, Nonce, BlockNumber> for $Runtime {
				...
				fn eth_transact(
					...
					let tx_fee = |call: <Self as $crate::frame_system::Config>::RuntimeCall, dispatch_call: <Self as $crate::frame_system::Config>::RuntimeCall| {
						...
						$crate::pallet_transaction_payment::Pallet::<Self>::compute_unadjusted_fee(
